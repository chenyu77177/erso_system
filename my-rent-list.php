...
<script>
    ...
    function getOrder(){
        ...
        fdata.append('getUserAllOrder', true);
                ...
                if(previou != order[i]["order_id"]){
                    ...
                    for(j = 0; j < trade_category.length; j++){
                        if(order[i]["trade_state"] == trade_category[j]["trade_state_id"]){
                            trade = trade_category[j]["user_show_name"];
                    ...
                    order_data += '    <td><a class="btn btn-primary" href="order-show.php?orderId=' + order[i]["order_id"] + '">檢視</a></td>';
                    order_data += '</tr>';
                }
