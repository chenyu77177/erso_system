...
<script>
    ...
    function getUser(){
        ...
        .then(function(data) {
            ...
            for(i = 0; i < user_data.length; i++){
                ...
                permission_list += '    <td><select id="permission_' + user_data[i]["user_id"] + '" class="form-select" aria-label="Default select example" required>';
    ...
    function update_permission(user_id){
        //取得標籤
        let permission = document.getElementById("permission_" + user_id);

        let fdata = new FormData();
        //透過表單送出資料(非同步傳輸)至後端處理程式
        fdata.append("user_id", user_id);
        fdata.append("permission", permission.value);
        fdata.append("updateUserPermission", true);
        fetch('includes/user-update.inc.php',
        {
            method: 'post',
            body: fdata
        })
